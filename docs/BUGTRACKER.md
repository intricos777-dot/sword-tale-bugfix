...
| ST-001b | **Watchdog-style death mid-game on well-provisioned systems**: the game boots, the intro passes, gameplay begins — then the crash lands while playing (usually on first encounter with a new area's material/shaders) | User report (local, 2026-09-21): "crashes after I can actually move past the intro" | BLK | EXTERNAL + DEV | Hypothesis: render thread stalls on **on-the-fly shader permutation compilation** for each new level/material. Warm the caches: `dxvk.enableGraphicsPipelineLibrary=True` + `dxvk.numCompilerThreads` + Steam shader pre-cache. First 1–2 full sessions build the DXVK state cache (`shadercache/3305630/DXVK_state_cache`). **Note:** on low-RAM hosts (this one included), observed mid-game deaths were NOT the watchdog but host RAM starvation (ST-001c) — no `UE4CC-*` dump was written. |
